...
class EmailAttachment(BaseModel):
    ...
    def validate_content_size(cls, v):
        ...
        if len(v) > max_size:
            raise ValueError(f"附件大小超过限制（最大10MB）")
        return v
...
class EmailDraftRequest(BaseModel):
    ...
    to: Optional[list[str]] = Field(None, description="收件人邮箱列表")
...
class EmailReplyRequest(BaseModel):
    ...
    to: Optional[list[str]] = Field(None, description="收件人邮箱列表（不填则默认回复给发送者/所有人）")
    subject: Optional[str] = Field(None, description="邮件主题（不填则自动添加 Re:）")
...
class EmailForwardRequest(BaseModel):
    ...
    reference_item_id: str = Field(..., description="原邮件ID")
    to: list[str] = Field(..., description="收件人邮箱列表")
    subject: Optional[str] = Field(None, description="邮件主题（不填则自动添加 Fwd:）")
...
class EmailSyncRequest(BaseModel):
    ...
    account_id: int = Field(..., description="账户ID")
    folder: str = Field("INBOX", description="文件夹名称")
    sync_state: Optional[str] = Field(None, description="同步状态")
    limit: int = Field(100, ge=1, le=500, description="获取数量")
    only_fields: Optional[list[str]] = Field(None, description="仅返回指定字段")
...
class FolderDetailItem(BaseModel):
    ...
    folder_class: Optional[str] = Field(None, description="文件夹类别 (e.g. IPF.Note)")
    total_count: int = Field(0, description="总项目数")
    unread_count: int = Field(0, description="未读项目数")
...
class AccountCreate(BaseModel):
    ...
    server: Optional[str] = Field(None, description="服务器地址（可选，使用默认）")
    domain: Optional[str] = Field(None, description="域名（可选，使用默认）")
...
class AccountUpdate(BaseModel):
    ...
    password: Optional[str] = Field(None, description="新密码（留空则不更新）")
...
class ApiKeyCreate(BaseModel):
    ...
    permissions: list[str] = Field(
        default=["send", "receive", "search"],
        description="权限列表"
    )
    allowed_accounts: list[int] = Field(
        default=[],
        description="允许使用的账户ID（空表示全部）"
    )
    ip_whitelist: list[str] = Field(
        default=[],
        description="IP白名单（空表示不限制）"
    )
    rate_limit: int = Field(100, ge=1, le=10000, description="每分钟请求限制")
    expires_days: Optional[int] = Field(None, description="过期天数（空表示使用默认）")
    remark: Optional[str] = Field(None, description="备注")
...
class TemplateSendRequest(BaseModel):
    ...
    template_name: Optional[str] = Field(None, description="模板名称（与template_id二选一）")
    account_id: int = Field(..., description="发送账户ID")
    to: list[str] = Field(..., description="收件人邮箱列表")
    variables: dict[str, str] = Field(default={}, description="变量值字典")
    cc: Optional[list[str]] = Field(None, description="抄送列表")
