...
class Settings(BaseSettings):
    ...
    # Exchange 邮件服务配置
    # =============================================================================
    EXCHANGE_SERVER: str = os.getenv("EXCHANGE_SERVER", "10.72.8.110")
    EXCHANGE_DOMAIN: str = os.getenv("EXCHANGE_DOMAIN", "hnanet")
    EXCHANGE_EMAIL_SUFFIX: str = os.getenv("EXCHANGE_EMAIL_SUFFIX", "@tianjin-air.com")
    # 凭据加密密钥（生产环境必须设置）
    ...
    EXCHANGE_STREAM_CONNECTION_TIMEOUT_MINUTES: int = int(
        os.getenv("EXCHANGE_STREAM_CONNECTION_TIMEOUT_MINUTES", "30")
    )
    # Streaming 异常重连等待（秒），仅在异常时生效
    EXCHANGE_STREAM_ERROR_RETRY_SECONDS: int = int(
        os.getenv("EXCHANGE_STREAM_ERROR_RETRY_SECONDS", "5")
    )
