...
from app.utils.crypto import generate_api_key, get_crypto, hash_api_key
...
class AccountService:
    ...
    async def test_account(self, account_id: int, owner_id: int) -> dict:
        ...
        try:
            ...
            async with get_exchange_connection(account_id) as conn:
                ...
                inbox_count = await loop.run_in_executor(None, lambda: conn.account.inbox.total_count)
    ...
    async def list_accounts(self, owner_id: int, page: int = 1, page_size: int = 20) -> dict:
        ...
        try:
            ...
            accounts = await ExchangeAccount.filter(owner_id=owner_id).offset((page - 1) * page_size).limit(page_size)

            items = [await acc.to_dict(exclude_fields=["encrypted_password"]) for acc in accounts]
    ...
    async def list_api_keys(self, owner_id: int, page: int = 1, page_size: int = 20) -> dict:
        ...
        try:
            ...
            keys = await ExchangeApiKey.filter(owner_id=owner_id).offset((page - 1) * page_size).limit(page_size)

            items = [await key.to_dict(exclude_fields=["key_hash"]) for key in keys]
    ...
    async def get_usage_stats(
        ...
        try:
            ...
            today_logs = [log for log in logs if log.created_at.strftime("%Y-%m-%d") == today_str]
    ...
    async def list_mail_logs(
        ...
        try:
            ...
            # 获取账户信息
            account_map = {acc.id: acc.email for acc in await ExchangeAccount.filter(id__in=accounts)}
