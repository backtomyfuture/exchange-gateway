...
from exchangelib.items import Contact as ExchContact
                ...
                def resolve_ops():
                    ...
                    if local_matches:
                        return [{
                            ...
                            "email": getattr(c, 'email_addresses', [None])[0].email_address if getattr(c, 'email_addresses', None) else None,
                            "mailbox_type": "Contact", 
                            "item_id": str(c.id) if c.id else None
                        } for c in local_matches]

                    # 2. 尝试 GAL (ResolveNames)
                    results = conn.account.protocol.resolve_names(
                        names=[query],
                    ...
                    for item in results:
                        ...
                        if isinstance(item, tuple):
                            ...
                            if contact:
                                ...
                                name = getattr(contact, 'display_name', None) or getattr(contact, 'complete_name', None) or getattr(mailbox, 'name', None)
