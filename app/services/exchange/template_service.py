...
class TemplateService:
    ...
    async def update_template(self, data: TemplateUpdate, owner_id: int) -> dict:
        ...
        """
        try:
            template = await ExchangeEmailTemplate.filter(id=data.id, owner_id=owner_id).first()
    ...
    async def delete_template(self, template_id: int, owner_id: int) -> dict:
        ...
        """
        try:
            template = await ExchangeEmailTemplate.filter(id=template_id, owner_id=owner_id).first()
    ...
    async def list_templates(
        ...
        try:
            ...
            templates = await query.order_by("-created_at").offset((page - 1) * page_size).limit(page_size)
