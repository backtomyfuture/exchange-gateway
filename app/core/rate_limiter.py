...
class RateLimiter:
    ...
    async def is_allowed(self, key: str, limit: int, window_seconds: int = 60) -> tuple[bool, int, int]:
        """
        检查请求是否被允许
        ...
        async with self._lock:
            # 清理过期的请求记录
            self._requests[key] = [(ts, count) for ts, count in self._requests[key] if ts > window_start]
    ...
    async def get_status(self, key: str, window_seconds: int = 60) -> dict:
        ...
        async with self._lock:
            current_requests = [(ts, count) for ts, count in self._requests.get(key, []) if ts > window_start]
