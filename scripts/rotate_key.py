...
async def main():
    ...
    parser.add_argument("--old-key", help="旧的加密密钥（默认从环境变量 EXCHANGE_ENCRYPTION_KEY 读取）")
    parser.add_argument("--new-key", required=True, help="新的加密密钥")
    parser.add_argument("--dry-run", action="store_true", help="验证模式：只检查旧密钥能否解密，不实际修改数据")
    ...
    try:
        ...
        if args.dry_run:
            print("\n💡 提示: 这是验证模式，数据未被修改")
            print("   确认无误后，移除 --dry-run 参数执行实际轮换")
        else:
            print("\n✅ 密钥轮换完成！")
            print("   请更新配置中的 EXCHANGE_ENCRYPTION_KEY 为新密钥")
