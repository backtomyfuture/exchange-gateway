...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    parent_id = result[1][0][0] if result[1] else None
    ...
    if parent_id:
        ...
        webhook_menu_id = result[1][0][0] if result[1] else None

        if webhook_menu_id:
            # Get role ID for "邮箱用户"
            result = await db.execute_query("SELECT `id` FROM `role` WHERE `name` = '邮箱用户' LIMIT 1")
            role_id = result[1][0][0] if result[1] else None
    ...
    old_parent_id = result[1][0][0] if result[1] else None

    if old_parent_id:
        # Check if has children
        result = await db.execute_query("SELECT COUNT(*) FROM `menu` WHERE `parent_id` = {}".format(old_parent_id))
