...
from app import app
from app.core.init_app import init_apis, init_menus, init_roles, init_superuser
...
async def superuser_token():
    ...
    payload = JWTPayload(user_id=user.id, username=user.username, is_superuser=user.is_superuser, exp=expire)
...
async def normal_user_token():
    ...
    if not user:
        user = await User.create(
            username="normal_user", email="normal@test.com", password="password", is_active=True, is_superuser=False
        )
