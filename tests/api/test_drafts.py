...
async def test_create_draft_success(client):
    ...
    with patch("app.api.v1.exchange.emails.get_email_service") as mock_svc_cls:
        ...
        mock_service.create_draft = AsyncMock(
            return_value={"success": True, "message": "Draft created", "id": "draft_123", "changekey": "ckpt_123"}
        )

        # Call API
        response = await client.post(
            "/api/v1/exchange/emails/drafts",
            json={"account_id": 1, "to": ["test@example.com"], "subject": "Test Draft", "body": "Body content"},
...
async def test_create_draft_forbidden(client):
    ...
    response = await client.post(
        "/api/v1/exchange/emails/drafts", json={"account_id": 1, "subject": "Test"}, headers={"X-API-KEY": raw_key}
    )
