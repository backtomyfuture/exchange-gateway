...
from app.services.exchange.connection_pool import ExchangeConnectionPool, ExchangeConnection, get_exchange_connection, _connection_pool
...
async def test_get_connection_creates_new(connection_pool, mock_account_db):
    ...
    with patch("app.services.exchange.connection_pool.get_connection_pool", return_value=connection_pool), \
         patch("app.services.exchange.connection_pool.ExchangeAccount.filter") as MockFilter, \
         patch("app.services.exchange.connection_pool.Account") as MockAccount, \
         patch("app.services.exchange.connection_pool.Configuration"), \
         patch("app.services.exchange.connection_pool.Credentials"):
        
        # Mock DB
        mock_qs = MagicMock()
        MockFilter.return_value = mock_qs
        mock_qs.first = AsyncMock(return_value=mock_account_db)
        
        # Mock Exchange Account
        mock_exch_acc = MagicMock()
        MockAccount.return_value = mock_exch_acc
